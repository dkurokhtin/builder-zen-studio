...
export const getUserInfo: RequestHandler = (req, res) => {
  ...
  const response = {
    ...user,
    daysLeft,
    totalDays: subscriptionPlans.find(p => p.id === user.planType)?.durationDays || 30
  };
...
export const getVpnConfig: RequestHandler = (req, res) => {
  ...
  const config: VpnConfig = {
    ...
    qrCode: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAQAAAAEAAQMAAABmvDolAAAABlBMVEX///8AAABVwtN+AAAHBUlEQVR42u...", // Мок QR
    serverInfo: {
      location: "Нидерланды",
      ping: "23ms",
      speed: "1000 Мбит/с",
...
export const extendSubscription: RequestHandler = (req, res) => {
  ...
  const currentEnd = user.subscriptionActive ? new Date(user.subscriptionEnd) : new Date();
  const newEnd = new Date(currentEnd.getTime() + (plan.durationDays * 24 * 60 * 60 * 1000));
